    ...
    window.liger = $.ligerui = {
        ...
        run: function (plugin, args, ext) {
            ...
            this.each(function () {
                ...
                if (ext.hasElement) {
                    //console.dir(ext.controlNamespace);
                    //console.dir(plugin);
                    //console.dir($.ligerui[ext.controlNamespace][plugin]);
...
(function ($) {
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _getParentNodeClassName: function (isOpen) {
            ...
            //var nodeclassname = 'glyphicon glyphicon-folder-close';
            //if (isOpen) nodeclassname = 'glyphicon glyphicon-folder-open';
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (g.hasChildren(o)) {
                    ...
                    if (p.parentIcon) {
                        ...
                        //如果是流程模型类别节点
                        if (o["isprocesstype"]) {
                            if (isExpandCurrent ? true : false) {
                                treehtmlarr.push('<div class="isprocesstype  processtreeiconcolor glyphicon glyphicon-folder-open"></div>')
                            } else {
                                treehtmlarr.push('<div class="isprocesstype  processtreeiconcolor glyphicon glyphicon-folder-close "></div>')
                            }
                        }
                        if (p.iconFieldName && o[p.iconFieldName]) {
                            //如果是流程节点
                            if (o["isprocess"]) {
                                treehtmlarr.push('<div class="' + o[p.iconFieldName] + '"></div>');
                            } else {
                    ...
                    if (p.childIcon) {
                        ...
                        if (o["isprocesstype"]) {
                            if (isExpandCurrent ? true : false) {
                                treehtmlarr.push('l-tree-icon-none isprocesstype  processtreeiconcolor glyphicon glyphicon-folder-open"')
                            } else {
                                treehtmlarr.push('l-tree-icon-none isprocesstype  processtreeiconcolor glyphicon glyphicon-folder-close ')
                            }
                        }
                        ...
                        if (p.iconFieldName && o[p.iconFieldName]) {
                            ...
                            //如果是流程模型节点
                            if (o["isdatamodel"] || o["ispicture"] || o["istablemodel"] || o["isformmodel"]) {
                                treehtmlarr.push('<div class="' + o[p.iconFieldName] + '"></div>');
                            } else {
                                treehtmlarr.push('<img src="' + o[p.iconFieldName] + '" />');
                            }
        ...
        _setTreeEven: function () {
            ...
            g.tree.click(function (e) {
                ...
                if ($(obj).hasClass("l-checkbox")) {
                    ...
                    //console.dir($(treeitembtn).next().find(".isprocesstype"));
                    $(treeitembtn).next().find(".isprocesstype").removeClass("glyphicon-folder-open").addClass("glyphicon-folder-close");
                    ...
                    $("> div ." + g._getParentNodeClassName(), treeitem)
                            ...
                            .addClass(g._getParentNodeClassName(true));
                    $(treeitembtn).next().find(".isprocesstype").removeClass("glyphicon-folder-close").addClass("glyphicon-folder-open");
                }
                // g.trigger('click', [{ data: treenodedata, target: treeitem[0] }]);
            });
